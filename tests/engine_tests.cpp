...
TEST(Engine, TransformComposition) {
    SceneTree tree;
    ...
    child->position = {10, 0};
    EXPECT_TRUE(near(child->globalPosition(), {100, 110}));

    parent->scale = {2, 2};
    EXPECT_TRUE(near(child->globalPosition(), {100, 120}));

    child->setGlobalPosition({50, 50});
    EXPECT_TRUE(near(child->globalPosition(), {50, 50}));
    ...
    EXPECT_TRUE(near(grandchild->globalPosition(), {100, 100}));
}
...
TEST(Engine, Lifecycle) {
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(tree.root().children().size() == 1);
    EXPECT_TRUE(tree.root().children()[0].get() == spawned);
    // Added during update, after the loop reached it by index: updated once.
    EXPECT_TRUE(spawned->updates == 1);

    tree.tick(0.016f);
    EXPECT_TRUE(spawned->updates == 2);
}

TEST(Engine, CollisionShapes) {
    using namespace collision;
    EXPECT_TRUE(overlaps(Circle{{0, 0}, 5}, Circle{{9, 0}, 5}));
    EXPECT_TRUE(!overlaps(Circle{{0, 0}, 5}, Circle{{11, 0}, 5}));

    OrientedBox box{{0, 0}, {1, 0}, {0, 1}, {5, 5}};
    EXPECT_TRUE(overlaps(Circle{{8, 0}, 4}, box));
    EXPECT_TRUE(!overlaps(Circle{{8, 8}, 4}, box));  // near the corner, but outside

    // 45-degree box whose AABB would overlap `box`, but which does not.
    const float s = std::sqrt(0.5f);
    OrientedBox diamond{{12, 12}, {s, s}, {-s, s}, {5, 5}};
    EXPECT_TRUE(!overlaps(box, diamond));
    diamond.center = {8, 8};
    EXPECT_TRUE(overlaps(box, diamond));

    // Via world transforms: rect rotated 45 degrees and scaled.
    const Shape rect = RectShape{{10, 10}};
    EXPECT_TRUE(!overlaps(rect, Transform2D{}, rect,
                          Transform2D::fromTRS({12, 12}, kPi / 4, {1, 1})));
    EXPECT_TRUE(overlaps(rect, Transform2D{}, rect,
                         Transform2D::fromTRS({12, 12}, kPi / 4, {2, 2})));
}

TEST(Engine, AreaEvents) {
    SceneTree tree;
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(entered == 0);

    b->position = {6, 0};
    tree.tick(0.016f);
    tree.tick(0.016f);
    EXPECT_TRUE(entered == 1);
    EXPECT_TRUE(a->overlapping().size() == 1);

    b->position = {100, 0};
    tree.tick(0.016f);
    tree.tick(0.016f);
    EXPECT_TRUE(exited == 1);
    EXPECT_TRUE(a->overlapping().empty());

    // Freeing an overlapping area drops it without an exit event.
    b->position = {6, 0};
    tree.tick(0.016f);
    EXPECT_TRUE(entered == 2);
    b->queueFree();
    tree.tick(0.016f);
    EXPECT_TRUE(exited == 1);
    EXPECT_TRUE(a->overlapping().empty());
...
TEST(Engine, LayerMask) {
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(bulletHits == 1);
    EXPECT_TRUE(enemyHits == 0);
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(bulletHits == 1);
    EXPECT_TRUE(ghost->overlapping().empty());
}

TEST(Engine, RenderQueueOrdering) {
    SceneTree tree;
    ...
    queue.flatten(verts);
    EXPECT_TRUE(verts.size() == 9);          // triangle + rect; hidden subtree skipped
    EXPECT_TRUE(verts.front().color[1] == 1);  // lower z first
    EXPECT_TRUE(verts.back().color[0] == 1);
}
...
public:
    ...
    void onSelf(Emitter& e) {
        EXPECT_TRUE(e.name == "emitter");
        ++selfCalls;
    }
};

TEST(Engine, SignalsImmediate) {
    SceneTree tree;
    ...
    EXPECT_TRUE(order.size() == 1 && order[0] == 30);
    EXPECT_TRUE(listener->values.size() == 1 && listener->values[0] == 3);
    ...
    sig.fire(1);
    EXPECT_TRUE(a == 1 && b == 0 && c == 1 && late == 0);
    EXPECT_TRUE(!ca.connected() && !cb.connected() && cc.connected());
    sig.fire(1);
    EXPECT_TRUE(a == 1 && c == 2 && late == 1);
    EXPECT_TRUE(sig.connectionCount() == 2);
    ...
    nested.fire(0);
    EXPECT_TRUE(depthCalls == 4);
    EXPECT_TRUE(nested.connectionCount() == 0);
}

TEST(Engine, SignalLifetime) {
    SceneTree tree;
    auto* emitter = tree.root().addChild<Emitter>();
    auto* listener = tree.root().addChild<Listener>();
    ...
    EXPECT_TRUE(emitter->value.connectionCount() == 0);
    ...
    emitter->queueFree();
    tree.tick(0.016f);
    EXPECT_TRUE(!fromDead.connected());
    ...
    parent->queueFree();
    tree.tick(0.016f);
    EXPECT_TRUE(!childConn.connected());
    emitter2->value.fire(5);
    EXPECT_TRUE(emitter2->value.connectionCount() == 0);
}

TEST(Engine, SignalsDeferred) {
    SceneTree tree;
    auto* emitter = tree.root().addChild<Emitter>();
    ...
    EXPECT_TRUE(listener->values.empty());
    tree.tick(0.016f);
    EXPECT_TRUE(listener->values.size() == 1 && listener->values[0] == 7);
    tree.tick(0.016f);
    EXPECT_TRUE(listener->values.size() == 1);
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(listener->selfCalls == 1);
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(received == "hello world");
    ...
    tree.tick(0.016f);
    EXPECT_TRUE(chained == 2);
    EXPECT_TRUE(direct);
}

TEST(Engine, ViewTransform) {
    View2D view;
    view.center = {10, -5};
    ...
    EXPECT_TRUE(near(view.visibleSize(), {40, 20}));
    view.screenSize = {400, 400};
    EXPECT_TRUE(near(view.visibleSize(), {20, 20}));
    view.screenSize = {800, 400};

    const Transform2D clip = view.worldToClip();
    EXPECT_TRUE(near(clip.apply(view.center), {0, 0}));
    EXPECT_TRUE(near(clip.apply(view.center + view.visibleSize() * 0.5f), {1, 1}));
    EXPECT_TRUE(near(clip.apply(view.center - view.visibleSize() * 0.5f), {-1, -1}));

    // Screen space: top-left pixel origin, y-down.
    EXPECT_TRUE(near(view.worldToScreen().apply(view.center), {400, 200}));
    EXPECT_TRUE(near(view.worldToScreen().apply({-10, -15}), {0, 0}));

    view.zoom = 2;
    EXPECT_TRUE(near(view.visibleSize(), {20, 10}));

    view.rotation = 0.7f;
    const Vec2 p{13, 2};
    EXPECT_TRUE(near(view.screenToWorld().apply(view.worldToScreen().apply(p)), p));
    EXPECT_TRUE(near(view.worldToClip().apply(view.center), {0, 0}));
}
...
TEST(Engine, Camera) {
    ...
    camera->zoom = 2;
    camera->makeCurrent();
    EXPECT_TRUE(tree.currentCamera() == camera);

    // The view follows the camera's position but stays upright.
    tree.tick(0.016f);
    EXPECT_TRUE(near(tree.view.center, camera->globalPosition()));
    EXPECT_TRUE(tree.view.rotation == 0.0f);
    EXPECT_TRUE(tree.view.zoom == 2.0f);

    camera->followRotation = true;
    tree.tick(0.016f);
    EXPECT_TRUE(std::abs(tree.view.rotation - 1.0f) < 1e-3f);
    ...
    EXPECT_TRUE(!camera->isCurrent());
    tree.tick(0.016f);
    EXPECT_TRUE(near(tree.view.center, {-7, 3}));

    // Freeing the current camera clears it; the view keeps its last values.
    fixed->queueFree();
    tree.tick(0.016f);
    EXPECT_TRUE(tree.currentCamera() == nullptr);
    EXPECT_TRUE(near(tree.view.center, {-7, 3}));
}
